...
4. **本地验证** — 推送前先本地自测，不要靠反复 push 让 CI 报错：
   - Debug **和 Release** 各构建 + 跑全量测试
   - 用与 CI 相同的条件跑测试：相同的 `QT_QPA_PLATFORM`（headless 用 `offscreen`；涉及原生窗口/样式的测试用真实平台）、尽量相同的 Qt 版本（版本差异会暴露只在 CI 出现的问题）
   - 仅平台特有、本机无法复现的问题（如 macOS SDK），先核实环境（Qt 版本可用性、SDK 变更）再推送
   - 全部通过才能推送；推送后尽量少 force-push（见「PR 规范 · 提交历史纪律」）
...
- 首行 ≤72 字符，末尾不加句号
- **关联 issue 写进 PR 描述，不写进 commit message**。`Closes #N` 若放在 commit 里，force-push 重写该 commit（SHA 变化）时，GitHub 会在 issue 时间线重复追加引用记录（见「PR 规范 · 提交历史纪律」）。commit 里如需提及，用 `Refs #N`
...
- **创建 PR 后**：应对审查意见优先**追加新 commit**，不要重写历史。
- **默认禁用 force-push，不到万不得已不使用**（如合并前 squash、修正严重错误）。确需使用时
  **只做一次**，做完立即停止。
- 原因：反复 force-push 让 reviewer 难以追踪改动、丢失评论定位、重复触发 CI；若 commit 带
  `Closes #N`，每次重写都会在 issue 时间线追加引用记录，且会永久污染主仓库记录。

### 谨慎远程提交（仅主仓库直接维护者）

本项目**直接操作主仓库**（非 fork 协作），远程提交要格外谨慎：

- 若预料提交可能不完美、推送后可能 CI 报错，**先不要直接 push**——先在本地验证清楚（见上方
  「本地验证」）。
- 一次不完美的 push 会**污染记录**：即使之后 force-push 或删除，CI 运行记录、issue 引用、
  commit 历史仍会保留，造成混乱。
- 不是禁止出错，而是**谨慎**：避免连续多次 push 每次都被 CI 报错。
